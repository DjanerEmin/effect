...
Lets imagine we want to study the effect of a new anti ADHD medication on patients ability to focus. In a placebo study we measure the focus duration of indivduals.
...
We have a fictitious experiment where it seems that the drug leads to increase of focus time.
...
Lets see if the difference between the treatment and the placebo is statistically significant assuming p \< 0.05.

```{r}
t.test(placebo, treatment)
```

Yes the `diff(c(mean_placebo, mean_treatment))` is around `r round(diff(c(mean_placebo, mean_treatment)))` minutes and is statistically significant because there is a miniscule (only `r round(100*t.test(placebo, treatment)$p.value, 4)` %) chance that those two sets (`placebo` and `treatment`) stem from the same underlying population.
...
The effect of this drug is `r round(cohensD(treatment, placebo),2)` times the pooled standard deviation of the study group - a very strong effect on focus time. On average, the drug leads to an increase in focus time by about (`d x pooled_sd`) `r round(cohensD(treatment, placebo) * pooled_sd)` minutes.
